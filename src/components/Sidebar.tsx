...
const Sidebar = () => {
    ...
    return (
        <div className="h-0">
            ...
            <div ref={sidebarRef} className={`bg-[#f7faf9] fixed h-full top-0 left-0 z-50 transition duration-300 ease-in-out ${isOpen ? "w-[80%]" : "w-50"} min-w-50 ${isOpen ? "translate-x-0" : "-translate-x-full"} md:translate-x-0 md:block`}>
                <div>
                    <Image src="/assets/logo.png" className="flex justify-center items-center pt-4 max-w-[160px] m-auto" width={160} height={60} alt="" />
                </div>
                <div className={`flex flex-col justify-between ${isPlayerRoute ? `h-[calc(95vh-60px)]` : "h-[calc(100vh-60px)]"} pb-5 overflow-y-auto`}>
                    <div className="flex-auto mt-10">
                        <div className="flex flex-col text-[#032b41] m-4 cursor-pointer">
                            <Link href="/forYou" className="flex py-5 relative" onClick={() => setActiveTab("/forYou")}>
                                <span className={`absolute top-0 h-full border-l-4 ${activeTab === "/forYou" ? "border-green-500" : "border-transparent"}`} style={{ left: "-16px", height: "60px" }}></span>
                                <PiHouseLine className="mr-2 text-2xl" /> For You
                            </Link>
                            <Link href="/Library" className="flex py-5 relative" onClick={() => setActiveTab("/Library")}>
                                <span className={`absolute top-0 h-full border-l-4 ${activeTab === "/Library" ? "border-green-500" : "border-transparent"}`} style={{ left: "-16px", height: "100%" }}></span>
                    ...
                    <div>
                        <Link href="/settings" className="flex ml-4 py-5 relative" onClick={() => setActiveTab("/settings")}>
                            <span className={`absolute top-0 h-full border-l-4 ${activeTab === "/settings" ? "border-green-500" : "border-transparent"}`} style={{ left: "-16px", height: "100%" }}></span>
            ...
            {isModalOpen && (
                <div className="fixed top-0 left-0 w-full h-full bg-black/40 z-[100] flex justify-center items-center">
                    <LoginModal onClose={closeLoginModal} onLogin={() => setIsLoggedIn(true)} />
                </div>
            )}
        </div>
    );
