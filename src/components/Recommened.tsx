      ...
      <div className="flex justify-center flex-wrap">
        {books.map((book) => (
          <Link
            key={book.id}
            href={`book/${book.id}`}
            className="w-[170px] pt-[32px] pr-[12px] pb-[12px] pl-[12px] rounded-sm hover:bg-[#f4f5f5d4] flex flex-col items-center"
          >
            <Image src={book.imageLink} width={170} height={100} alt={book.title} />
            <h2 className="font-bold text-[#032b41] mb-2 line-clamp-2 w-[150px] text-center">
              {book.title}
            </h2>
